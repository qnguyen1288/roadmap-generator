...
// THE TEXT SIZE
$name_width = get_width(100,0,$fontBold,$Name);
...
imagettftext($img, 100, 0, $centerX, 200, $black, $fontBold, $Name);
...
for($i=0; $i <= (count($output_array)-1); $i++) {
    ...
    $textarrow1_width = get_width(37,0,$font,$textarrow1);
    ...
    imagettftext($img, 37, 0, $textX, 1320,$black, $font, $textarrow1); // dien text 1
    unset($textX);
    $textarrow2_width = get_width(37,0,$font,$textarrow2);
    $textX = ceil($start + $gap*($itemarrow-1) - $textarrow2_width/2 +25);
    $textX = $textX <0 ? 0 : $textX;
    imagettftext($img, 37, 0, $textX, 1375,$black, $font, $textarrow2); // dien text 2
    unset($textX);
    if($i != (count($output_array)-1)) {
        ...
        if ($tgian2[$i] > 12) {
            ...
            imagettftext($img, 35,0, $start + $gap*(($output_array[$i+1]+$itemarrow-2)/2) - ($timetext_width/2) +25, 1165, $pink, $font, $timetext);
        } else {
...
$line1_width = get_width(40,0,$font,$line1);
...
$line2_width = get_width(40,0,$font,$line2);

// CENTERING THE TEXT BLOCK
$centerX = CEIL(($width - $line1_width) / 2);
$centerX = $centerX<0 ? 0 : $centerX;
imagettftext($img, 40, 0, $centerX, 1550, $black, $font, $line1);
imagettftext($img, 40, 0, $centerX, 1550, $pink, $font, $line1x);

$centerX = CEIL(($width - $line2_width) / 2);
$centerX = $centerX<0 ? 0 : $centerX;
imagettftext($img, 40, 0, $centerX, 1605, $black, $font, $line2);
